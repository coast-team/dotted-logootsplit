...
import { fromArray, FromPlain, isObject } from "../util/data-validation"
import { isU32, u32 } from "../util/number"
import { Ordering } from "../util/ordering"
import { BaseBlockk, Block, BlockOrdering, LengthBlock } from "./block"
import { BlockFactory, BlockFactoryConstructor } from "./block-factory"
...
const block3wayComparison = <E extends Concat<E>>(ref: BaseBlockk) => (
    b: BaseBlockk
) => {
    if (ref.lowerPos.compare(b.lowerPos) === Ordering.AFTER) {
...
export class BlockList<E extends Concat<E>> extends OpReplicatedList<E> {
    ...
    remove(dBlock: LengthBlock): Del[] {
        const it = this.blocks.atEqual(block3wayComparison(dBlock), true)
        const rmv = []
...
export class EditableBlockList<E extends Concat<E>>
    ...
    at(sIndex: u32): ListIterator<Block<E>> {
